...
const YT_QUERY = "learn languages for kids";
...
export default function LanguagesChatPage() {
  ...
  useEffect(() => {
    const key = `languages_chat_${params.childId}`;
    ...
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "You are a friendly language tutor for kids. Help with learning new languages, use fun emojis, and make language learning exciting and interactive!" },
            ...messages,
            userMsg
          ],
          max_tokens: 512,
      ...
      <div className="w-full max-w-5xl flex flex-col md:flex-row gap-8 bg-white rounded-xl shadow-lg border border-[#d2dce4]">
        ...
        <div className="flex-1 flex flex-col p-6">
          <header className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-lg font-bold text-[#0f151a]">Foreign Languages Chat</h2>
          </header>
          <div className="flex-1 flex flex-col gap-2 mb-2">
            {messages.length === 0 && (
              <div className="text-gray-400 text-center">Ask me about learning languages!</div>
            )}
            {messages.map((msg, i) => (
          ...
          <form onSubmit={sendMessage} className="flex gap-2 mt-2 items-center">
            <input
              ...
              type="text"
              placeholder="Type your language question..."
              value={input}
        ...
        {/* YouTube Videos */}
        <div className="flex-1 flex flex-col p-6">
          <h3 className="text-xl font-bold mb-4 text-[#0f151a]">Explore Language Videos</h3>
          ...
          <a href="https://www.youtube.com/results?search_query=learn+languages+for+kids" target="_blank" rel="noopener noreferrer">

...
const YT_QUERY = "reading and writing for kids";
...
export default function ReadingChatPage() {
  ...
  useEffect(() => {
    const key = `reading_chat_${params.childId}`;
    ...
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "You are a friendly reading and writing tutor for kids. Help with reading comprehension, writing prompts, encourage creativity, and use fun emojis in your responses to make it engaging for children!" },
            ...messages,
            userMsg
          ],
          max_tokens: 512,
      ...
      <div className="w-full max-w-5xl flex flex-col md:flex-row gap-8 bg-white rounded-xl shadow-lg border border-[#d2dce4]">
        ...
        <div className="flex-1 flex flex-col p-6">
          <header className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-lg font-bold text-[#0f151a]">Reading & Writing Chat</h2>
          </header>
          <div className="flex-1 flex flex-col gap-2 mb-2">
            {messages.length === 0 && (
              <div className="text-gray-400 text-center">Ask me about reading or writing!</div>
            )}
            {messages.map((msg, i) => (
          ...
          <form onSubmit={sendMessage} className="flex gap-2 mt-2 items-center">
            <input
              ...
              type="text"
              placeholder="Type your reading or writing question..."
              value={input}
        ...
        <div className="flex-1 flex flex-col p-6">
          ...
          <a href="https://www.youtube.com/results?search_query=reading+and+writing+for+kids" target="_blank" rel="noopener noreferrer">

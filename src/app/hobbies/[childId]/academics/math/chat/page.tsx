...
export default function MathChatPage() {
  ...
  useEffect(() => {
    const key = `math_chat_${params.childId}`;
    ...
    try {
      ...
      const res = await fetch(API_ENDPOINTS.GROQ_CHAT, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "You are a friendly math tutor for kids. Explain answers step by step, use simple language, encourage learning, and use fun emojis in your responses to make it engaging for children!" },
            ...messages,
            userMsg
          ],
          max_tokens: 512,
      ...
      <div className="w-full max-w-5xl flex flex-col md:flex-row gap-8 bg-white rounded-xl shadow-lg border border-[#d2dce4]">
        ...
        <div className="flex-1 flex flex-col p-6">
          <header className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-lg font-bold text-[#0f151a]">Math Tutor Chat</h2>
          </header>
          <div className="flex-1 flex flex-col gap-2 mb-2">
            {messages.length === 0 && (
              <div className="text-gray-400 text-center">Ask me any math question!</div>
            )}
            {messages.map((msg, i) => (
        ...
        {/* YouTube Videos */}
        <div className="flex-1 flex flex-col p-6">
          <h3 className="text-xl font-bold mb-4 text-[#0f151a]">Explore Math Videos</h3>
          ...
          <a href="https://www.youtube.com/results?search_query=math+for+kids" target="_blank" rel="noopener noreferrer">
